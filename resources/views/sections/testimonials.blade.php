<section id="testimonials" class="section-padding testimonials-bg relative" data-animate>
    <div class="container mx-auto px-4 relative z-10">
        ...
        <div x-data="{
            ...
            <div class="glass-card rounded-4xl p-8 lg:p-14 relative overflow-hidden border border-border/50 hover:border-primary/30 transition-all duration-500 group">
                ...
                <div class="relative">
                    ...
                    <div class="flex justify-center items-center gap-6 mt-10">
                        <button @click="prev()"
                                class="w-14 h-14 rounded-full bg-muted hover:bg-primary/20 flex items-center justify-center transition-all duration-300 border border-border hover:border-primary/50 hover:scale-110 active:scale-95">
                        ...
                        <div class="flex items-center gap-3">
                            <template x-for="(testimonial, index) in testimonials" :key="index">
                                <button @click="currentIndex = index"
                                        ...
                                        class="h-3 rounded-full transition-all duration-300 hover:scale-120"></button>
                            </template>
                        </div>
                        <button @click="next()"
                                class="w-14 h-14 rounded-full bg-muted hover:bg-primary/20 flex items-center justify-center transition-all duration-300 border border-border hover:border-primary/50 hover:scale-110 active:scale-95">
            ...
            <div class="grid md:grid-cols-2 gap-6 mt-10 max-w-2xl mx-auto">
                <template x-for="(testimonial, index) in testimonials" :key="index">
                    <button @click="currentIndex = index"
                            :class="index === currentIndex ? 'border-primary shadow-glow bg-primary/5' : 'border-border/50 hover:border-primary/30'"
                            class="glass-card rounded-2xl p-5 text-left transition-all duration-500 border relative overflow-hidden group hover:-translate-y-1 hover:translate-x-1">
                        {{-- Green and Blue gradient background on hover --}}
